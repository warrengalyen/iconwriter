...
pub(crate) fn svg(source: &Tree, size: u32) -> Result<DynamicImage, ResReResampleError> {
...
pub(crate) fn apply<F: FnMut(&DynamicImage, u32) -> io::Result<DynamicImage>>(
    ...
    if dims != (size, size) {
        Err(ResReResampleError::MismatchedDimensions(size, dims))
    } else {
        Ok(icon)
    }
}
